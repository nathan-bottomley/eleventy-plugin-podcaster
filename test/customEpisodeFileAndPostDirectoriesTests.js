...
test.before(async (t) => {
  const eleventy = new Eleventy(
    './fixtures/customEpisodeFileAndPostDirectories/src',
    './fixtures/customEpisodeFileAndPostDirectories/_site',
    {
      configPath: './fixtures/customEpisodeFileAndPostDirectories/eleventy.config.js'
    })
  t.context.build = await eleventy.toJSON()
  t.context.feed = t.context.build.find(
    item => item.inputPath === './fixtures/customEpisodeFileAndPostDirectories/src/feed.njk'
  )
  t.context.episodePosts = t.context.build.filter(
    item => item.inputPath.startsWith('./fixtures/customEpisodeFileAndPostDirectories/src/posts/')
  )
})
...
test.after.always(async (t) => {
  const episodeDataPath = path.join(process.cwd(), 'fixtures/customEpisodeFileAndPostDirectories/src/_data/episodeData.json')
  const podcastDataPath = path.join(process.cwd(), 'fixtures/customEpisodeFileAndPostDirectories/src/_data/podcastData.json')
  try {
