...
test.before(async (t) => {
  t.context.fixture = './fixtures/customEpisodePostsDirectory'
  ...
  t.context.feed = t.context.build.find(
    item => item.inputPath === `${t.context.fixture}/feed.njk`
  )
  t.context.episodePosts = t.context.build.filter(
    item => item.inputPath.startsWith(`${t.context.fixture}/custom-episode-posts/`)
  )
})
